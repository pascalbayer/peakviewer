...
There is deliberately **no LICENSE file** — the terms are the owner's call, not
something a build should assume. Add one before publishing if you want people to
be able to use this.

Third-party obligations are met in `THIRD-PARTY-NOTICES.md`, regenerated by the
build. Everything bundled is Apache-2.0 or MIT, none of it copyleft. Note that
Apache-2.0 section 4 requires retaining licence notices in redistributed code,
so the bundles are built with `legalComments: 'eof'` rather than stripping them.
